...
PRODUCTS: Dict[str, Dict[str, Any]] = {
    "insclaw_full": {
        ...
        "price_fen": 9900,  # ¥99
        "description": "购买后即可下载完整版（含 Chromium / Node / Python 运行时，约 1GB，开箱即用）或轻量版（约 70MB，安装时联网拉取依赖）。",
    },
}
...
def landing_create_order(body: CreateOrderBody, request: Request, db: Session = Depends(get_db)):
    ...
    return {
        "out_trade_no": out_trade_no,
        "amount_fen": int(charge_fen),
        "amount_yuan": round(charge_fen / 100, 2),
        "product_id": body.product_id,
