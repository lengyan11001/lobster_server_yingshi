...
async def chat_endpoint(
    ...
    sys_prompt = (
        ...
        + (
            ...
            "【爆款TVC/带货视频】用户说「TVC」「爆款TVC」「带货视频」「做TVC」时，不走video.generate，改用 invoke_capability(\"comfly.daihuo.pipeline\", {action:\"start_pipeline\", asset_id:\"素材ID\", auto_save:true})。该流水线的子任务 task_id(video_开头)禁止调task.get_result，MCP会自动轮询。\n"
            "图生视频: 用户附图时只填prompt/model/duration，禁止填image_url（系统自动注入）\n"
            "图片编辑(AI重绘): image.generate + image_url=附件URL，model用edit系列(wan/v2.7/edit等)，prompt中用 image 1 引用\n"
            "素材剪辑(改画幅/裁剪/叠字/静音/配乐): media.edit + operation + asset_id。「画幅改成9:16」= media.edit + operation:scale_pad + aspect_ratio:9:16，禁止用image.generate代替。\n"
    ...
    async def run_chat() -> None:
        ...
        sys_prompt = (
            ...
            + (
                ...
                "【爆款TVC/带货视频】用户说「TVC」「爆款TVC」「带货视频」「做TVC」时，不走video.generate，改用 invoke_capability(\"comfly.daihuo.pipeline\", {action:\"start_pipeline\", asset_id:\"素材ID\", auto_save:true})。该流水线的子任务 task_id(video_开头)禁止调task.get_result。\n"
                "图生视频: 用户附图时只填prompt/model/duration，禁止填image_url（系统自动注入）\n"
