...
async def test_precreate(biz_content, label=''):
...
async def main():
    ...
    # Test 1: with type_id=1 (WeChat)
    await test_precreate({
        'merchant_order_sn': tsn + '_wx',
        'total_amount': 0.01,
    ...
    # Test 2: with type_id=2 (Alipay)
    await test_precreate({
        'merchant_order_sn': tsn + '_ali',
        'total_amount': 0.01,
        'store_id': int(store_id) if store_id else None,
        'body': 'test',
        'type_id': 2,
    ...
    # Test 3: no type_id (聚合)
    await test_precreate({
        'merchant_order_sn': tsn + '_agg',
        'total_amount': 0.01,
        'store_id': int(store_id) if store_id else None,
        'body': 'test',
        'notify_url': 'https://example.com/notify',
    ...
    # Test 4: amount=1 (1 yuan)
    await test_precreate({
        'merchant_order_sn': tsn + '_1y',
        'total_amount': 1.00,
        'store_id': int(store_id) if store_id else None,
        'body': 'test 1yuan',
        'notify_url': 'https://example.com/notify',
