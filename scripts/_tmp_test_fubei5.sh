...
echo "=== 测试 fbpay.fixed.qrcode.create ==="
.venv/bin/python3 -c "
...
async def main():
    ...
    # Test fbpay.fixed.qrcode.create
    await test_api('fbpay.fixed.qrcode.create', {
        'merchant_order_sn': tsn + '_fqc',
    ...
    # Test fbpay.order.create with pay_type
    await test_api('fbpay.order.create', {
        'merchant_order_sn': tsn + '_oc_wx',
        'store_id': int(store_id),
        'total_amount': 0.01,
        'body': 'test',
        'pay_type': 'wxpay',
        ...
        'fail_url': 'https://example.com/fail',
        'cancel_url': 'https://example.com/cancel',
    }, 'order.create pay_type=wxpay')

    # Test fbpay.order.create with pay_type=aggregate
    await test_api('fbpay.order.create', {
        'merchant_order_sn': tsn + '_oc_agg',
        'store_id': int(store_id),
        'total_amount': 0.01,
        'body': 'test',
        'pay_type': 'aggregate',
        ...
        'fail_url': 'https://example.com/fail',
        'cancel_url': 'https://example.com/cancel',
    }, 'order.create pay_type=aggregate')
