...
echo "=== 上传 custom_configs.json ==="
scp $SSH_OPTS "$ROOT/custom_configs.json" "$LOBSTER_DEPLOY_HOST:$REMOTE_DIR/custom_configs.json"
echo "上传完成"

echo "=== 验证远端文件 ==="
ssh $SSH_OPTS "$LOBSTER_DEPLOY_HOST" "cat $REMOTE_DIR/custom_configs.json"

echo ""
echo "=== 重启服务 ==="
ssh $SSH_OPTS "$LOBSTER_DEPLOY_HOST" "cd $REMOTE_DIR && bash scripts/server_update_and_restart.sh"
