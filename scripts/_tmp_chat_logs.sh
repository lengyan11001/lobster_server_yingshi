...
echo "=== 服务器 backend.log 最近 chat 相关 ==="
ssh $SSH_OPTS "$LOBSTER_DEPLOY_HOST" bash -s <<'REMOTE'
cd /opt/lobster-server
echo "--- backend.log chat/deepseek/sutui ---"
grep -iE "chat|deepseek|direct|xskill|route|timeout|error|sutui-chat|chat_trace" backend.log 2>/dev/null | tail -60
echo ""
echo "--- journalctl chat 相关 ---"
journalctl -u lobster-backend --no-pager --since "10 minutes ago" 2>/dev/null | grep -iE "chat|deepseek|sutui|direct|route|timeout|error" | tail -40
REMOTE
